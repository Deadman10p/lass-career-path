...
interface ResultsRow {
  ...
  scoresByCluster: Record<string, number>;
}
...
export default function SetterResults() {
    ...
    (async () => {
      ...
      const [{ data: results }, { data: profiles }] = await Promise.all([
        ...
        studentIds.length
          ? supabase
              .from("profiles")
              .select("user_id, full_name, class_name, stream")
      ...
      const built: ResultsRow[] = (resps ?? []).map((r: any) => {
        ...
        myResults.forEach((x: any) => {
          map[x.career_cluster_id] = x.total_score;
        });
        ...
        return {
          ...
          questionnaire_title:
            qMap.get(r.questionnaire_id) ?? "Untitled questionnaire",
          submitted_at: r.submitted_at,
          topCluster: top
            ? cs.find((c) => c.id === top.career_cluster_id)?.name ?? null
            : null,
  ...
  const streams = useMemo(
    () => Array.from(new Set(rows.map((r) => r.stream).filter(Boolean))) as string[],
    [rows]
  );
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (questionnaireFilter !== ALL && r.questionnaire_id !== questionnaireFilter)
        return false;
  ...
  const topClusterCounts = useMemo(() => {
    ...
    filtered.forEach((r) => {
      if (r.topCluster) m[r.topCluster] = (m[r.topCluster] ?? 0) + 1;
    });
  ...
  const exportCsv = () => {
    ...
    for (const r of filtered) {
      lines.push(
        [
          ...
          esc(r.class_name ?? ""),
          esc(r.stream ?? ""),
          esc(r.questionnaire_title),
          new Date(r.submitted_at).toISOString(),
          esc(r.topCluster ?? ""),
          String(r.topScore ?? ""),
          ...clusters.map((c) => String(r.scoresByCluster[c.id] ?? 0)),
        ].join(",")
      );
    }
    ...
    a.download = `lass-career-results-${stamp}.csv`;
    a.click();
    ...
    <PageShell tone="setter" title="Counsellor Portal">
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            All career inventory submissions across the school. Filter by class
            and stream to focus your review.
          </p>
        </div>
        <Button
          onClick={exportCsv}
          size="sm"
          className="gradient-setter text-setter-foreground border-0 shadow-glow"
        >
      ...
      <div className="mb-5 grid gap-3 sm:grid-cols-3">
        ...
        <StatCard
          ...
          label="Most common top cluster"
          valueLabel={topClusterCounts[0]?.[0] ?? "—"}
          loading={loading}
        />
        <StatCard
          ...
          value={
            new Set(filtered.map((r) => r.class_name).filter(Boolean)).size
          }
          loading={loading}
        />
      </div>
      ...
      <div className="mb-4 rounded-2xl border border-border bg-card p-4 shadow-card">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Select
            ...
            onValueChange={setQuestionnaireFilter}
          >
            <SelectTrigger>
            ...
            >
              <div className="flex items-center justify-between border-b border-border bg-secondary/40 p-4 sm:p-5">
                <div className="flex items-center gap-3">
                  <div className="flex h-9 w-9 items-center justify-center rounded-lg gradient-setter text-setter-foreground text-xs font-semibold">
                    {list.length}
                  </div>
                  <div>
                      ...
                      <TableRow key={r.response_id}>
                        ...
                        <TableCell className="text-right">
                          <Button asChild size="sm" variant="outline">
                            <Link
                              to={`/setter/questionnaire/${r.questionnaire_id}/analytics`}
                            >
                              View
                            </Link>
                          </Button>
